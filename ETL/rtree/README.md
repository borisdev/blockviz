...
A user types some address into the browser. The problem here is to match his address input to a blockgroup offset so we can send the user demographic and other stats for the blockgroup containing that address

## Technical Approach

Using scripts here pre-build a client-side rtree data-structure that is based on the RBush library and the blockgroup bounding boxes of a census shapefile

## Steps to build a JSON R-Tree

### Step 1

blocks_boundingbox.py --- use python script to get bounding boxes of block group shapefile

### Step 2 

census_blocks_rtree.js ---
using serverside node compliler with RBush library to output a JSON R-Tree, which we will then put in the static directory
